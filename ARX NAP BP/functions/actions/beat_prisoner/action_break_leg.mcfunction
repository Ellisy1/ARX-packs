...
scoreboard players set @p[tag=!self, r=3, hasitem={item=arx:shackles, location=slot.armor.legs}] ill_broken_leg 72000
damage @p[tag=!self, r=3, hasitem={item=arx:shackles, location=slot.armor.legs}] 2 suicide
execute as @s[tag=!nearest_player_has_shackles] run tellraw @s { "rawtext": [ { "text": "§6Чтобы сломать ногу игроку в кандалах, сначала надо подойти к нему." } ] }
