# ЭТА ФУНКЦИЯ ЗАПУСКАЕТСЯ ПРЕДМЕТОМ ДЛЯ ПОВЫШЕНИЯ ЛОВКОСТИ

# Если камера не поднята rxm=-89
    tellraw @s[rxm=-89] { "rawtext": [ { "text": "§d1 очко обучения§f: Возвысить §aЛОВКОСТЬ\n§eБонусы для нечетных очков:\n§e>§f +1 скорость копания\n§e>§f +1 переносимый вес\n§e>§f -30 сек к интервалу между авторегенрерацией\n§eБонусы для четных очков:\n§e>§f +1 стабильность стрельбы из лука\n§e>§f +1 скорость\n§e>§f +1 прыжок" } ] } 
...
# Если камера поднята rx=-89
    ...
    # Есть невложенные очки
        execute as @s[rx=-89, scores={skill_point=1..}] run title @s title §aЛовкость возвышена!
        execute as @s[rx=-89, scores={skill_point=1..}] run tellraw @s { "rawtext": [ { "text": "§aЛовкость возвышена!" } ] } 
        execute as @s[rx=-89, scores={skill_point=1..}] run scoreboard players add @s speed_skill 1
        execute as @s[rx=-89, scores={skill_point=1..}] run clear @s arx:agility_up 0 1
