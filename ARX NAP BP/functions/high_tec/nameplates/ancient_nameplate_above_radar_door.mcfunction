...
# Недопуск
    tellraw @s[scores={battery_avail=0}] { "rawtext": [ { "text": "§cНа табличке изображено нечто, посыл чего вам абсолютно неясен." } ] }
    tellraw @s[scores={battery_avail=1}, tag=!descriptor_equipped] { "rawtext": [ { "text": "§cНа табличке изображено нечто, посыл чего вам абсолютно неясен." } ] }
...
# Успех
    execute as @s[tag=has_enough_charge, tag=descriptor_equipped] run tellraw @s { "rawtext": [ { "text": "§f[§bПереведено§f>>> §o<Вероятно, символ радиосигнала> (Мелкая надпись ниже) Военный объект. Вход без допуска категорически запрещён.§r§f]" } ] }
