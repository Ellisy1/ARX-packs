...
# Успех
    execute as @s[tag=has_enough_charge, tag=descriptor_equipped] run tellraw @s { "rawtext": [ { "text": "§f[§bПереведено§f>>> §oДобро пожаловать в магазин электроники! (Мелкая надпись ниже) Универсальный код для связи и трассировки электричества: D-0-7-8-2-1-3-3-2-2-9§r§f]" } ] }
