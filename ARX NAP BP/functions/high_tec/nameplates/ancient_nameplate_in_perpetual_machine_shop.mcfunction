...
# Успех
    execute as @s[tag=has_enough_charge, tag=descriptor_equipped] run tellraw @s { "rawtext": [ { "text": "§f[§bПереведено§f>>> §oДобро пожаловать в магазин батарей и зарядных утройств <Вечный двигатель>!§r§f]" } ] }
