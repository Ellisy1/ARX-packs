...
# Недопуск
    ...
    tellraw @s[scores={sk_a_26=1, battery_avail=0}] { "rawtext": [ { "text": "§cНа листочке явно что-то написано, но язык вам абсолюно незнаком." } ] }
    tellraw @s[scores={sk_a_26=1, battery_avail=1}, tag=!descriptor_equipped] { "rawtext": [ { "text": "§cНа листочке явно что-то написано, но язык вам абсолюно незнаком." } ] }
...
# Успех
    execute as @s[scores={sk_a_26=1}, tag=has_enough_charge, tag=descriptor_equipped] run tellraw @s { "rawtext": [ { "text": "§f[§bПереведено§f>>> §oПароль от трассировщика A-B-C-1-2-3-4-5-F. Вы утомили меня спрашивать, так что теперь это будет висеть здесь.§r§f]" } ] }
