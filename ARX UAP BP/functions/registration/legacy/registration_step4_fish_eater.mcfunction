...
execute @p[tag=self, rx=-89] ~ ~ ~ tellraw @p { "rawtext": [{ "text": "§aЗадан вкус - любитель рыбы!" } ] } 
execute @p[tag=self, rx=-89] ~ ~ ~ function food/presets/fish_eater
...
execute @p[tag=self, rxm=-89] ~ ~ ~ tellraw @p { "rawtext": [{ "text": "§fВаш персонаж будет гораздо больше любить рыбу, чем что-либо ещё." } ] } 
execute @p[tag=self, rxm=-89] ~ ~ ~ tellraw @p { "rawtext": [{ "text": "§8§oПоднимите камеру вертикально вверх и используйте этот предмет ещё раз, чтобы подтвердить, что §l§aваш вкус - любитель рыбы." } ] } 
