...
execute @p[tag=self, rx=-89] ~ ~ ~ tellraw @p { "rawtext": [{ "text": "§aЗадан вкус - нейтральный!" } ] } 
execute @p[tag=self, rx=-89] ~ ~ ~ function food/presets/neutral
...
execute @p[tag=self, rxm=-89] ~ ~ ~ tellraw @p { "rawtext": [{ "text": "§fВаш персонаж не будет иметь любимой пищи. Все вкусы будут случайно заданы от -20 до 20." } ] } 
execute @p[tag=self, rxm=-89] ~ ~ ~ tellraw @p { "rawtext": [{ "text": "§8§oПоднимите камеру вертикально вверх и используйте этот предмет ещё раз, чтобы подтвердить, что §l§aваш вкус - нейтральный." } ] } 
