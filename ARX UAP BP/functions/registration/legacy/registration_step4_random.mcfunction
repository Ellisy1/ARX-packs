...
execute @p[tag=self, rx=-89] ~ ~ ~ tellraw @p { "rawtext": [{ "text": "§aЗадан вкус - случайный!" } ] } 
execute @p[tag=self, rx=-89] ~ ~ ~ function food/presets/random
...
execute @p[tag=self, rxm=-89] ~ ~ ~ tellraw @p { "rawtext": [{ "text": "§fВсе вкусы вашего персонажа будут случайно выбраны в диапазоне от -100 до 100. §cНе рекомендуется выбирать!" } ] } 
execute @p[tag=self, rxm=-89] ~ ~ ~ tellraw @p { "rawtext": [{ "text": "§8§oПоднимите камеру вертикально вверх и используйте этот предмет ещё раз, чтобы подтвердить, что §l§aваш вкус - случайный." } ] } 
