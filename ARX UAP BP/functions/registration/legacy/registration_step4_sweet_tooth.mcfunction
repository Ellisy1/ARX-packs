...
execute @p[tag=self, rx=-89] ~ ~ ~ tellraw @p { "rawtext": [{ "text": "§aЗадан вкус - сладкоежка!" } ] } 
execute @p[tag=self, rx=-89] ~ ~ ~ function food/presets/sweet_tooth
...
give @p[tag=self, rx=-89] arx:registration_step5_ready 1 0


#Действия в нормальной позиции
execute @p[tag=self, rxm=-89] ~ ~ ~ tellraw @p { "rawtext": [{ "text": "§fВаш персонаж будет гораздо больше любить сладкое и хлебное, чем что-либо ещё." } ] } 
execute @p[tag=self, rxm=-89] ~ ~ ~ tellraw @p { "rawtext": [{ "text": "§8§oПоднимите камеру вертикально вверх и используйте этот предмет ещё раз, чтобы подтвердить, что §l§aваш вкус - сладкоежка." } ] } 
