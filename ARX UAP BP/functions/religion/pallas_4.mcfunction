...
tellraw @p[tag=self, rxm=-89] { "rawtext": [ { "text": "§6Адепт Палласа§f -> §cМаг Палласа" } ] }
tellraw @p[tag=self, rxm=-89] { "rawtext": [ { "text": "§6Требования: 60 уровень, 6 уровень АS&S." } ] }
tellraw @p[tag=self, rxm=-89, scores={religion=3}] { "rawtext": [ { "text": "§8§oПоднимите камеру вертикально вверх и используйте этот предмет ещё раз, чтобы повысить уровень религии до Мага Палласа." } ] }
...
execute @p[tag=self, tag=success] ~ ~ ~ tellraw @p { "rawtext": [{ "text": "§2Паллас наделяет Вас силой мага!" } ] }
execute @p[tag=self, tag=success] ~ ~ ~ title @p subtitle §cМаг Палласа!
execute @p[tag=self, tag=success] ~ ~ ~ title @p title §eНовый уровень:
execute @p[tag=self, tag=success] ~ ~ ~ clear @p arx:pallas_4 0 1
execute @p[tag=self, tag=success] ~ ~ ~ xp -60L @p
execute @p[tag=self, tag=success] ~ ~ ~ scoreboard players set @p religion 4
