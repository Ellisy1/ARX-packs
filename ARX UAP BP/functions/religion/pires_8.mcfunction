...
tellraw @p[tag=self, rxm=-89] { "rawtext": [ { "text": "§6Адепт Пиреса§f -> §cРейнджер Пиреса" } ] }
tellraw @p[tag=self, rxm=-89] { "rawtext": [ { "text": "§6Требования: 60 уровень, 6 уровень АS&S." } ] }
tellraw @p[tag=self, rxm=-89, scores={religion=13}] { "rawtext": [ { "text": "§8§oПоднимите камеру вертикально вверх и используйте этот предмет ещё раз, чтобы повысить уровень религии до Рейнджера Пиреса." } ] }
...
execute @p[tag=self, tag=success] ~ ~ ~ tellraw @p { "rawtext": [{ "text": "§2Пирес наделяет Вас силой рейнджера!" } ] }
execute @p[tag=self, tag=success] ~ ~ ~ title @p subtitle §cРейнджер Пиреса!
execute @p[tag=self, tag=success] ~ ~ ~ title @p title §eНовый уровень:
execute @p[tag=self, tag=success] ~ ~ ~ clear @p arx:pires_8 0 1
execute @p[tag=self, tag=success] ~ ~ ~ xp -60L @p
execute @p[tag=self, tag=success] ~ ~ ~ scoreboard players set @p religion 18
