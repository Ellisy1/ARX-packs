#Анализ доступности
    scoreboard players set @p mp_req 12
...
#Заклинание
    execute @p[tag=self, tag=spell_available, scores={target=1}] ~ ~ ~ summon arx:small_rat_black ~ ~ ~

    execute @p[tag=self, tag=spell_available, scores={target=2}] ~ ~ ~ execute @p[tag=self2] ~ ~ ~ summon arx:small_rat_black ~ ~ ~
