#Анализ доступности
    scoreboard players set @p mp_req 25

    tag @p[tag=self, tag=staff_din] add drop_cost_with_staff
    tag @p[tag=self, scores={religion=22}] add drop_cost_with_religion

    function spells/operations/drop_spell_cost
    function spells/operations/check_mp_spell_availability

#Заклинание
    execute @p[tag=self, tag=spell_available, scores={target=1}] ~ ~ ~ summon arx:small_rat_white ~ ~ ~ become_agressive

    execute @p[tag=self, tag=spell_available, scores={target=2}] ~ ~ ~ execute @p[tag=self2] ~ ~ ~ summon arx:small_rat_white ~ ~ ~ become_agressive

    execute @p[tag=self, tag=spell_available, scores={target=3}] ~ ~ ~ w @p кажется, это заклинание так не работает
    tag @p[tag=self, tag=spell_available, scores={target=3}] add cant_be_casted_cus_of_target

    function spells/operations/withdraw_mp