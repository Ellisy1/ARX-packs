#Анализ доступности
    scoreboard players set @p mp_req 210
...
#Заклинание
    execute @p[tag=self, tag=spell_available, scores={target=1}] ~ ~ ~ effect @a[r=15] night_vision 300 0 true

    execute @p[tag=self, tag=spell_available, scores={target=2}] ~ ~ ~ effect @a[r=15, tag=!self] night_vision 300 0 true
