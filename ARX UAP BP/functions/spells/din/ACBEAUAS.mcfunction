#Анализ доступности
    scoreboard players set @p mp_req 360
...
#Заклинание
    execute @p[tag=self, tag=spell_available, scores={target=1}] ~ ~ ~ effect @p night_vision 1800 0 true

    execute @p[tag=self, tag=spell_available, scores={target=2}] ~ ~ ~ effect @p[tag=self2] night_vision 1800 0 true
