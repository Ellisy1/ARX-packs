#Анализ доступности
    scoreboard players set @p mp_req 70
...
#Заклинание
    execute @p[tag=self, tag=spell_available, scores={target=1}] ~ ~ ~ effect @p resistance 900 255 true
    execute @p[tag=self, tag=spell_available, scores={target=1}] ~ ~ ~ effect @p invisibility 900 255 true
    execute @p[tag=self, tag=spell_available, scores={target=1}] ~ ~ ~ effect @p slowness 900 255 true
