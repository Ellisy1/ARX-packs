...
#Заклинание
    execute @p[tag=self, tag=spell_available, scores={target=1}] ~ ~ ~ effect @p[tag=self, scores={pois_block=0}] fatal_poison 30 0
    execute @p[tag=self, tag=spell_available, scores={target=1}] ~ ~ ~ particle arx:poison_a ~ ~2 ~

    execute @p[tag=self, tag=spell_available, scores={target=2}] ~ ~ ~ effect @p[tag=self2, scores={pois_block=0}] fatal_poison 30 0
    execute @p[tag=self, tag=spell_available, scores={target=2}] ~ ~ ~ execute @p[tag=self2] ~ ~ ~ particle arx:poison_a ~ ~2 ~

    execute @p[tag=self, tag=spell_available, scores={target=3}] ~ ~ ~ effect @e[type=!player, type=!item, r=15, scores={pois_block=0}] fatal_poison 30 0
