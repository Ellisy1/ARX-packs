...
#Заклинание
    execute @p[tag=self, tag=spell_available, scores={target=1}] ~ ~ ~ effect @a[r=15] instant_health 1 255 

    execute @p[tag=self, tag=spell_available, scores={target=2}] ~ ~ ~ effect @a[tag=!self, r=15] instant_health 1 255
